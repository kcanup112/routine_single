...
def create_subject(subject: schemas.SubjectCreate, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
    try:
...
def get_subjects(skip: int = 0, limit: int = 1000, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
    return SubjectService.get_all(db, skip, limit)

@router.get("/{subject_id}/", response_model=schemas.Subject)
def get_subject(subject_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
...
def update_subject(subject_id: int, subject: schemas.SubjectUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
...
def delete_subject(subject_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
