...
def get_teacher_subjects(teacher_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
...
def get_available_subjects(teacher_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
...
def assign_subject_to_teacher(teacher_id: int, subject_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
...
def remove_subject_from_teacher(teacher_id: int, subject_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
...
def get_teachers_by_subject(subject_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
