...
def create_programme(programme: schemas.ProgrammeCreate, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
    return ProgrammeService.create(db, programme)

@router.get("/", response_model=List[schemas.Programme])
def get_programmes(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
    return ProgrammeService.get_all(db, skip, limit)

@router.get("/{programme_id}/", response_model=schemas.Programme)
def get_programme(programme_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
...
def get_programmes_by_department(department_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_read_access)):
    return ProgrammeService.get_by_department(db, department_id)

@router.put("/{programme_id}/", response_model=schemas.Programme)
def update_programme(programme_id: int, programme: schemas.ProgrammeUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
...
def delete_programme(programme_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
