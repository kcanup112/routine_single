...
def create_day(day: schemas.DayCreate, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
...
def delete_day(day_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_write_access)):
